# the r script builds the master one-row-per-peak table for HepG2 HNF1A peaks (GSM6248577)
...
# output is saved as GSM6248577_peakcentric.csv
...
peaks <- readPeakFile("/Users/Manu/Downloads/GSE206240_RAW/HNF1A/GSM6248577_HepG2_HNF1A_ab96777_peaks.bed")
...
for (i in seq_len(nrow(GSM6248577))) {
  if (!is.null(tss_list_entrez[[as.character(i)]])) {
    GSM6248577$genes_TSS_10kb[i] <- paste(entrez_to_symbol(tss_list_entrez[[as.character(i)]]), collapse = ",")
  }
  if (!is.null(body_list_entrez[[as.character(i)]])) {
    GSM6248577$genes_geneBodyOverlap_10kb[i] <- paste(entrez_to_symbol(body_list_entrez[[as.character(i)]]), collapse = ",")}}
...
# join onto GSM6248577 by peak_id
GSM6248577 <- merge(
  GSM6248577,
...
names(GSM6248577)[names(GSM6248577) == "annotation"] <- "peak_overlap_type"
names(GSM6248577)[names(GSM6248577) == "distanceToTSS"] <- "peak_distanceToTSS"

# simplified overlap category for summary statistics
GSM6248577$peak_overlap_simple <- dplyr::case_when(
  is.na(GSM6248577$peak_overlap_type)                  ~ NA_character_,
  grepl("Promoter",  GSM6248577$peak_overlap_type)    ~ "Promoter",
  grepl("5' UTR",    GSM6248577$peak_overlap_type)    ~ "5UTR",
  grepl("3' UTR",    GSM6248577$peak_overlap_type)    ~ "3UTR",
  grepl("Exon",      GSM6248577$peak_overlap_type)    ~ "Exon",
  grepl("Intron",    GSM6248577$peak_overlap_type)    ~ "Intron",
  grepl("Downstream",GSM6248577$peak_overlap_type)    ~ "Downstream",
  grepl("Intergenic",GSM6248577$peak_overlap_type)    ~ "Intergenic",
  TRUE                                                 ~ "Other")

# match peak names for FIMO analysis
GSM6248577 <- GSM6248577 %>%
  mutate(sequence_name = gsub(":", "_", peak_id))

# add upstream bin (subset of intergenic peaks upstream of promoter)
GSM6248577$peak_overlap_custom <- dplyr::case_when(
  is.na(GSM6248577$peak_overlap_simple) ~ NA_character_,
  
  # keep existing annotations
  GSM6248577$peak_overlap_simple != "Intergenic" ~ GSM6248577$peak_overlap_simple,
  
  # redefine subset of intergenic peaks as upstream of promoter
  GSM6248577$peak_overlap_simple == "Intergenic" &
    !is.na(GSM6248577$peak_distanceToTSS) &
    GSM6248577$peak_distanceToTSS >= -3000 &
    GSM6248577$peak_distanceToTSS < -2000 ~ "Upstream",
...
# save output
write.csv(GSM6248577, "HNF1A_HepG2_GSM6248577/GSM6248577_Outputs/GSM6248577_peakcentric.csv", row.names = FALSE)
