# this r script tests islet HNF1A ChIP-Seq peak score against motif presence and motif count
...
# read the per-peak motif table written by GSM6248576_FIMO.R
all_peaks_with_counts <- read_csv("HNF1A_Islets_GSM6248576/GSM6248576_Outputs/GSM6248576_FIMO_AllPeaks.csv", show_col_types = FALSE)
...
set.seed(42)
plot_islets_binary <- ggplot(all_peaks_with_counts, aes(x = motif_group, y = log_peak_score)) +
  ...
  labs(x = "HNF1A Motif Presence",
       ...
       title = "GSM6248576 (Islets): Peak Score By Motif Presence",
...
ggplot(all_peaks_with_counts, aes(x = motif_count_group, y = log_peak_score)) +
  ...
  labs(x = "Number of HNF1A Motifs Per Peak",
       y = bquote(bold(log[10]*"(Peak Score + 1)")),
       title = "GSM6248576 (Islets): Peak Score By Motif Count",
