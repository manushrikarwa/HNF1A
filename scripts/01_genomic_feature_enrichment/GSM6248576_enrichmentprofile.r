# this r script computes the genomic-feature distribution of islet HNF1A peaks (GSM6248576),
# tests enrichment against a genome background: observed vs expected per category
# output saved to GSM6248576_Enrichment_Profile.csv
...
final_enrichment_table <- data.frame(
  ...
  exp_n = as.integer(exp_counts),
  exp_pct = round(100 * exp_prob, 2)) |>
  left_join(category_enrichment_results, by = "category")
...
write.csv(enrichment_out, "HNF1A_Islets_GSM6248576/GSM6248576_Outputs/GSM6248576_Enrichment_Profile.csv", row.names = FALSE)
