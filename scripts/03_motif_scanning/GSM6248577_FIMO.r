# this r script parses the HepG2 FIMO MA0046.1 output, reconstructs orientation-aware
...
GSM6248577_FIMO <- GSM6248577_FIMO %>%
  rename(p_value = `p-value`, q_value = `q-value`)

# read the original FASTA file to pull sequences from
GSM6248577_FASTA <- readDNAStringSet("HNF1A_HepG2_GSM6248577/GSM6248577_Outputs/GSM6248577_peaks-FASTAs.fa")
...
GSM6248577_FIMO <- GSM6248577_FIMO %>%
  rowwise() %>%
  mutate(
    full_seq = as.character(GSM6248577_FASTA[[sequence_name]]),
...
distinct_counts <- GSM6248577_FIMO %>%
...
table(GSM6248577_FIMO %>% distinct(sequence_name, motif_hits_per_peak) %>% pull(motif_hits_per_peak))
table(distinct_counts$distinct_motifs)

# join distinct-site counts onto the full peak list (peaks with no hit -> 0)
all_peaks_with_counts <- GSM6248577 %>%
...
# save output
write.csv(all_peaks_with_counts, "HNF1A_HepG2_GSM6248577/GSM6248577_Outputs/GSM6248577_FIMO_AllPeaks.csv", row.names = FALSE)