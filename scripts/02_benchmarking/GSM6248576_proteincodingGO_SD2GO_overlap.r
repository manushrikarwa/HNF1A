...
all_peaks <- read_csv("HNF1A_Islets_GSM6248576/GSM6248576_Outputs/GSM6248576_peakcentric.csv", show_col_types = FALSE)
...
GSM6248576_GO_nearest_proteincoding <- enrichGO(
  gene          = all_entrez,
  OrgDb         = org.Hs.eg.db,
  keyType       = "ENTREZID",
  ont           = "BP",
...
my_GO <- as.data.frame(GSM6248576_GO_nearest_proteincoding)
paper_ids <- unique(na.omit(trimws(GSM6248576_SD2_GO$ID)))
...
paper_only_terms <- GSM6248576_SD2_GO %>%
  filter(ID %in% paper_only_ids) %>%
  arrange(p.adjust)

# recovery of published terms
recovery_islets <- length(intersect(my_ids, paper_ids)) / length(paper_ids) * 100
recovery_islets

# write the all-peaks protein-coding GO
write.csv(my_GO, "HNF1A_Islets_GSM6248576/GSM6248576_Outputs/GSM6248576_ProteinCodingGO_AllPeaks.csv", row.names = FALSE)
